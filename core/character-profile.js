...
function buildCharacterTarget(profile, settings, requestedName = null, initialProgress = false) {
  ...
  try {
    characterCurrent = initialProgress ? { level: 1, levelLimit: 20 } : requireProgress(profile.character, '角色等级', true);
...
function requireProgress(value, label, character = false) {
  const level = value?.level;
  const levelLimit = value?.levelLimit;
  if (!Number.isInteger(level) || !Number.isInteger(levelLimit)) {
    throw new Error(`${label}档案字段不完整；请确认 OCR V6 后重新识别`);
  }
  if (character && level >= 90 && level <= 100 && [90, 95, 100].includes(levelLimit) && level <= levelLimit) {
